...
vec3 getNormal(vec2 texCenter, float dir) {
  vec2 pixFrac = dir/texture_size;
  vec2 tx = texCenter.xy; //center
  vec3 p = vec3(tx, readDepth(tx));
  tx = texCenter.xy+ vec2(0.0, pixFrac.y); //up
  vec3 p1 = vec3(tx, readDepth(tx));
  tx = texCenter.xy + vec2(pixFrac.x, 0.0); //right
  vec3 p2 = vec3(tx, readDepth(tx));
  vec3 normal = cross(p2-p, p1-p);
  return normalize(normal);
}

/*vec3 getNormal2(vec2 texCenter) {
	vec2 pixFrac = 0.5/texture_size;
	vec3 n1 = getNormal(texCenter+pixFrac, -1.0);
	vec3 n2 = getNormal(texCenter-pixFrac, 1.0);
