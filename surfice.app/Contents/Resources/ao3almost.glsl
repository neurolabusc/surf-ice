...
void main(void) {
  ...
  if (t1.a == 0.0) discard;
  //texture_size
  vec3 normalAtOrigin = getNormal(texCoord);
  vec3 worldSpaceOrigin = worldFromScreen(texCoord);
  color.rgb = normalAtOrigin;
  ...
  for (int sampleNumber = 0; sampleNumber < NUM_SAMPLES; sampleNumber++) {
      ...
      vec3 worldSpaceSample = getOffsetPositionVS(texCoord, angleUnitVector, sampleDistance);
	  ao += worldSpaceSample.z;
      // Step 3:
  ...
  color.rgb = vec3(occlusion,ao,ao);
  return;
