...
The MZ3 format is the internal file format for Surfice. The goals are to be simple, fast and small.
 ...
 - The total file size in bytes is `FILESIZE = 16 + NSKIP + isMATRIX * 64 + isFACE * NFACE * 12 + isVERT * NVERT * 12 + isRGBA * NVERT * 4 + isSCALAR * NSCALAR * NVERT * 4`.
 ...
 - If isMATRIX, a spatial transformation matrix is stored. This 4x4 matrix is composed of 32-bit floats and allows one to transform the input vertex positions to spatial position in mm. This would allow the raw vertex positions to store vertex position in different units (e.g. voxel grid position of the vertex). This is optional, without isMATRIX the vertices are already expected to be in mm space. The matrix is stored to disk with columns (c) before rows (r): m[c0,r0], m[c0,r1]...m[c3,r3].
...
Faces indexed from 0: a triangle of the first 3 vertices is  0,1,2
 ...
 HEADER: first 16 bytes
  ...
  2-3: UINT16 : ATTR attributes bitfield reporting stored data (value larger than 15 indicates future version):
        ...
        bitand(ATTR,16) = 16 : isMATRIX File includes spatial transformation matrix
 ...
 The total file size in bytes is
 	FILESIZE = 16+NSKIP+isMATRIX*64+isFACE*NFACE*12+isVERT*NVERT*12+isRGBA*NVERT*4+isSCALAR*NSCALAR*NVERT*4
 ...
 Note: for better compression integer data is transposed (interleaved)
  MATRIX: if isMATRIX next 12 * 4 bytes
   (if isMATRIX is false, assume identity matrix [no transformation required])
   (same as NIfTI sform: OutX=mat[0,0]*inX+mat[1,0]*inY+mat[2,0]+mat[3,0])
   +0..3: FLOAT32: mat[c0, r0]
   +4..7: FLOAT32: mat[c0, r1]
   +8..11: FLOAT32: mat[c0, r2]
   +12..15: FLOAT32: mat[c0, r3]
   +16..19: FLOAT32: mat[c1, r0]
    ....
   +64..63: FLOAT32: mat[c3, r3]
