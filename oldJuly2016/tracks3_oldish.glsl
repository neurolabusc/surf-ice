...
void main() {
	...
	if(vType==0.0){//Part of the Track Cylinders
		VectorBillboard = VectorBillboard*Radius*0.001;
		if(kind==0.0){
		...
		vec4 VectorEndBillboard = normalize(vec4(cross(VectorBillboard.xyz,VectorPoints.xyz),0.0))*Radius*0.001;
		VectorBillboard = VectorBillboard*Radius*0.001;
		if(kind==0.0){
...
void main() {
	...
	vec3 L = normalize(vec3(0.0, 0.0, 1.2));
	vec3 n = NN;
	float NormalSpec = pow(dot(vN,L),1.0);
	float BillboardSpec = pow(max(0.0,dot(N.xyz,L)), Shininess) * Specular;
	float dif = dot(L,n);
	color = vec4(NormalSpec*BillboardSpec*specClr + difClr*dif + 0.4*ambClr , 1.0);
	color *= 0.0;
}
