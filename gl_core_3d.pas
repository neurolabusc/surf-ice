...
+#10'uniform mat3 NormalMatrix;'
+#10'uniform vec3 LightPos = vec3(0.0, 20.0, 30.0);'
+#10'out vec4 vClr;'
+#10'out vec4 vP;'
+#10'out vec3 vN;'
...
+#10'out mat3 Rot;'
+#10'mat3 AlignVectors(vec3 FromVector,vec3 ToVector){'
...
+#10'	mat3 final = mat3(1.0)+vX+vX*vX*(1-c)/s*s;'
+#10'	return final;'
+#10'}'
...
+#10'	Rot=AlignVectors(vec3(0.0,-1.0,0.0),VectorPoints);'
+#10'}';
...
+#10'	}'
+#10'	vec3 NN =  Rot*normalize(2.0*N.xyz-1.0);'
...
+#10'	float dif = dot(n,L) * Diffuse;'
+#10'	vec3 h = normalize(vec3(0.0, 0.35, 0.5));'
+#10'	float s = pow(max(0.0,dot(n,h)), Shininess) * Specular * 2.0;'
+#10'	color = vec4(s + difClr*dif + Ambient*ambClr , 1.0);'
+#10'	color = vec4(difClr*dif + Ambient*ambClr, 1.0);'
+#10'}';
